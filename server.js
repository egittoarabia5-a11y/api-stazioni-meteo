...
app.get("/station/limet/:id.html", async (req, res) => {
  ...
  try {
    const response = await fetch(`https://api-stazioni-meteo.vercel.app/limet/${id}.json`);
    if (!response.ok) throw new Error("Stazione non trovata");
    const data = await response.json();const html = `
    <!DOCTYPE html>
    <html lang="it">
    <head>
    <meta charset="UTF-8">
    <title>${data.N || id} — Gauges</title>
    ...
    /* iOS-style card */
    .ios-card { max-width:600px; margin:20px auto; background:#fff; border-radius:20px; box-shadow:0 5px 20px rgba(0,0,0,0.1); padding:16px; font-family:Inter,sans-serif; color:#333; }
    .ios-card h2 { margin:0; font-size:20px; font-weight:600; }
    .ios-card p { margin:2px 0; font-size:14px; color:#666; }
    .ios-flex { display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; }
    .ios-temp { display:flex; justify-content:space-between; background:#f0f0f0; border-radius:16px; padding:12px; margin-bottom:12px; }
    .ios-temp-left { display:flex; flex-direction:column; }
    .ios-temp-left p:first-child { font-size:32px; font-weight:700; margin:0; }
    .ios-temp-left p:last-child { font-size:14px; color:#3b82f6; display:flex; align-items:center; gap:4px; margin:2px 0 0 0; }
    .ios-temp-right p { margin:2px 0; font-size:14px; }
    .ios-rain { display:flex; justify-content:space-between; background:#e0f2fe; border-radius:16px; padding:12px; margin-bottom:12px; }
    .ios-rain-left { display:flex; align-items:center; gap:8px; }
    .ios-rain-left p:first-child { font-size:18px; font-weight:600; margin:0; }
    .ios-rain-left p:last-child { font-size:12px; color:#666; margin:0; }
    .ios-rain-right p { font-size:12px; color:#555; margin:2px 0; }
    .ios-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:8px; text-align:center; }
    .ios-grid div { background:#f0f0f0; border-radius:16px; padding:8px; }
    .ios-grid div p:first-child { font-size:12px; color:#666; margin:0; }
    .ios-grid div p:last-child { font-size:16px; font-weight:600; margin:2px 0 0 0; }
    </style>
    </head>
    <body>
    <!-- iOS-style card -->
    <div class="ios-card">
      <div class="ios-flex">
        <div>
          <h2>${data.N || "Stazione"}</h2>
          <p>${data.altitude ?? "—"} m • ${data.source ?? "Protezione Civile"}</p>
        </div>
        <span class="text-xs text-gray-400">⏱ ${data.updateTime ?? "N/D"}</span>
      </div>
    
      <div class="ios-temp">
        <div class="ios-temp-left">
          <p>${data.T ?? "—"}°</p>
          <p>💧 ${data.H ?? "—"}%</p>
        </div>
        <div class="ios-temp-right">
          <p style="color:#f87171">▲ ${data.TH ?? "—"}° <span style="color:#999">(Max)</span></p>
          <p style="color:#3b82f6">▼ ${data.TL ?? "—"}° <span style="color:#999">(Min)</span></p>
        </div>
      </div>
    
      <div class="ios-rain">
        <div class="ios-rain-left">
          <span>🌧️</span>
          <div>
            <p>${data.R ?? "—"} mm</p>
            <p>Intensità: ${data.RR ?? "—"} mm/h</p>
          </div>
        </div>
        <div class="ios-rain-right">
          <p>📅 Giorno: <span>${data.R ?? "—"} mm</span></p>
          <p>📅 Mese: <span>${data.Rmonth ?? "—"} mm</span></p>
        </div>
      </div>
    ...
    <!-- GAUGES -->
    <header>
      <h1 id="stationTitle">${data.N || id}</h1>
    ...
    <div class="grid">
      <div class="card">
        <h3>Temperatura (°C)</h3>
        <canvas id="g_temp" width="260" height="260"></canvas>
        <div class="row">
          <div class="meta">Dew point: <b id="dewpt">${data.D ?? "—"}</b> °C</div>
          <div class="meta">Percepita: <b id="feels">${data.heat ?? data.T ?? "—"}</b> °C</div>
        </div>
      </div>
      <div class="card">
        <h3>Dew Point (°C)</h3>
        <canvas id="g_dew" width="260" height="260"></canvas>
      </div>
      <div class="card">
        <h3>Umidità (%)</h3>
        <canvas id="g_rh" width="260" height="260"></canvas>
        <div class="row">
          <div class="meta">UR: <b id="rh_txt">${data.H ?? "—"}</b> %</div>
          <div class="meta">UV: <b id="uv">${data.uv ?? "—"}</b></div>
        </div>
      </div>
      <div class="card">
        <h3>Pressione (hPa)</h3>
        <canvas id="g_press" width="260" height="260"></canvas>
        <div class="row">
          <div class="meta">Trend: <b id="ptrend">${data.pTrend ?? "—"}</b></div>
          <div class="meta">QNH: <b id="p_txt">${data.P ?? "—"}</b> hPa</div>
        </div>
      </div>
      <div class="card">
        <h3>Vento (km/h)</h3>
        <canvas id="g_wind" width="260" height="260"></canvas>
        <div class="row">
          <div class="meta">Dir: <b id="wdir">${data.W ?? "—"}</b>°</div>
          <div class="meta">Raffica: <b id="gust">${data.G ?? "—"}</b> km/h</div>
        </div>
      </div>
      <div class="card">
        <h3>Pioggia totale (mm)</h3>
        <canvas id="g_rain" width="260" height="260"></canvas>
        <div class="row">
          <div class="meta">Giorno: <b id="rtotal">${data.RR ?? 0}</b> mm</div>
        </div>
      </div>
      <div class="card">
        <h3>Rain Rate (mm/h)</h3>
        <canvas id="g_rainrate" width="260" height="260"></canvas>
      </div>
    ...
    <footer>Gauges © SteelSeries Canvas · Dati da LIMET</footer>
    
    <script>
    // ----- CONFIG GAUGES -----
    const tempSections = [
      steelseries.Section(-50,-10, "#1c1f4a"), steelseries.Section(-10,-5, "#1a3bbf"),
      steelseries.Section(-5,0, "#5a85a0"), steelseries.Section(0,5, "#a0c4d4"),
      steelseries.Section(5,11, "#2f4d2f"), steelseries.Section(11,15, "#5a8a3f"),
      steelseries.Section(15,20, "#c0a840"), steelseries.Section(20,25, "#c07030"),
      steelseries.Section(25,30, "#bf4040"), steelseries.Section(30,35, "#5c0c0c"),
      steelseries.Section(35,38, "#c06ca0"), steelseries.Section(38,50, "#6b4bbc")
    ];
    const humSections = [
      steelseries.Section(0,10,"#F01E00"), steelseries.Section(10,20,"#F07D00"),
      steelseries.Section(20,30,"#F0CB00"), steelseries.Section(30,40,"#F0B800"),
      steelseries.Section(40,50,"#3AA300"), steelseries.Section(50,60,"#21C573"),
      steelseries.Section(60,70,"#21C2C5"), steelseries.Section(70,80,"#00BDEF"),
      steelseries.Section(80,90,"#0060F0"), steelseries.Section(90,100,"#0114F0")
    ];
    const rainSections = [
      steelseries.Section(0,0.2,"#808080"), steelseries.Section(0.2,5,"#F09F00"),
      steelseries.Section(5,10,"#006400"), steelseries.Section(10,20,"#3AA300"),
      steelseries.Section(20,30,"#87CEEB"), steelseries.Section(30,50,"#4682B4"),
      steelseries.Section(50,100,"#0038F0"), steelseries.Section(100,150,"#FF69B4"),
      steelseries.Section(150,500,"#843BEB")
    ];
    const windSections = [
      steelseries.Section(0,20,"#00BFFF"), steelseries.Section(20,50,"#00FF00"),
      steelseries.Section(50,100,"#FF0000")
    ];
    const pressSections = [
      steelseries.Section(950,980,"#FF0000"), steelseries.Section(980,1010,"#00FF00"),
    ...
    function dynamicRange(value,buffer=10,minLimit=null,maxLimit=null){
      if(value===null||isNaN(value)) return {min:minLimit??0,max:maxLimit??100};
      let min=Math.floor(value-buffer), max=Math.ceil(value+buffer);
      if(minLimit!==null) min=Math.max(min,minLimit);
      if(maxLimit!==null) max=Math.min(max,maxLimit);
      if(max-min<buffer*2){min=value-buffer; max=value+buffer;
        if(minLimit!==null) min=Math.max(min,minLimit);
        if(maxLimit!==null) max=Math.min(max,maxLimit);}
      return {min,max};
    }
    function updateGaugeWithDynamicScale(gauge,value,options={buffer:10,min:null,max:null}){
      if(value===null||value===undefined) return;
      const range=dynamicRange(value,options.buffer,options.min,options.max);
      gauge.setMinValue(range.min); gauge.setMaxValue(range.max);
      gauge.setValueAnimated(value);
    }
    
    // ----- CREAZIONE GAUGES -----
    const commonRadial={gaugeType:steelseries.GaugeType.TYPE4,frameDesign:steelseries.FrameDesign.TILTED_GRAY,backgroundColor:steelseries.BackgroundColor.BEIGE,pointerVisible:false,ledVisible:false,lcdVisible:true,lcdDecimals:1,tickLabelOrientation:steelseries.TickLabelOrientation.HORIZONTAL,niceScale:true};
    ...
    function updateGauges(data){
      updateGaugeWithDynamicScale(gTemp, data.temp ?? null, {buffer:15});
      updateGaugeWithDynamicScale(gDew, data.dewp ?? null, {buffer:10});
      updateGaugeWithDynamicScale(gRH, data.hum ?? null, {min:0,max:100});
      updateGaugeWithDynamicScale(gPress, data.pres ?? null, {buffer:15});
      if(data.wind!==null){gWind.setMinValue(Math.max(0,data.wind-15)); gWind.setMaxValue(data.wind+15); gWind.setValueAnimated(data.wind);}
      if(data.windGust!==null){gWind.setMinValue(Math.max(0,data.windGust-15)); gWind.setMaxValue(data.windGust+15); gWind.setValueAnimated(data.windGust);}
      updateGaugeWithDynamicScale(gRain, data.rtot ?? null, {min:0});
      if(data.rrate!==null){gRainRate.setMinValue(Math.max(0,data.rrate-50)); gRainRate.setMaxValue(data.rrate+50); gRainRate.setValueAnimated(data.rrate);}
      // testi
      setText("dewpt", fmt1(data.dewp));
      setText("feels", fmt1(data.heat??data.temp));
      setText("rh_txt", fmt1(data.hum));
      setText("uv", data.uv ?? "—");
      setText("p_txt", fmt1(data.pres));
      setText("ptrend", data.pTrend ?? "—");
      setText("gust", fmt1(data.windGust));
      setText("wdir", fmt1(data.windDir));
      setText("rtotal", fmt1(data.rtot));
    }
    
    // ----- INIT -----
    function initGauges(){
      updateGauges({ temp: ${data.T ?? null}, dewp: ${data.D ?? null}, hum: ${data.H ?? null}, pres: ${data.P ?? null}, wind: ${data.V ?? null}, windGust: ${data.G ?? null}, windDir: ${data.W ?? null}, rtot: ${data.RR ?? null}, rrate: ${data.R ?? null}, heat: ${data.heat ?? data.T ?? null}, uv: "${data.uv ?? "—"}", pTrend: "${data.pTrend ?? "—"}" });
    }
    
    // Aggiornamento automatico ogni 10 secondi
    setInterval(() => { initGauges(); document.getElementById("updated").textContent = "Aggiornamento: " + new Date().toLocaleTimeString(); }, 10000);
    ...
    res.status(500).send(`<h1>Errore caricamento stazione</h1><p>${err.message}</p>`);
  }
});
