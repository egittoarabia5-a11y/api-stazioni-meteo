...
// Definizione stazioni Weathercloud
const stationsWeathercloud = {
  "San Martino": {lat: 44.408, lon: 8.978, code: "8840835265"}
};
...
app.get('/weathercloud.json', async (req, res) => {
  try {
    ...
    for (const stName in stationsWeathercloud) {
      const st = stationsWeathercloud[stName];
      const url = `https://app.weathercloud.net/device/stats?code=${st.code}`;
      ...
      // Helper per estrarre valore e timestamp
      const getVal = (obj) => (Array.isArray(obj) ? parseFloat(obj[1]) : null);
      const getTime = (obj) => (Array.isArray(obj) ? obj[0] * 1000 : null);

      const temp = getVal(data.temp_current);
      const tempMax = getVal(data.temp_day_max);
      const tempMin = getVal(data.temp_day_min);

      const dew = getVal(data.dew_current);
      const dewMax = getVal(data.dew_day_max);
      const dewMin = getVal(data.dew_day_min);

      const hum = getVal(data.hum_current);
      const humMax = getVal(data.hum_day_max);
      const humMin = getVal(data.hum_day_min);

      const press = getVal(data.bar_current);
      const pressMax = getVal(data.bar_day_max);
      const pressMin = getVal(data.bar_day_min);

      const wind = getVal(data.wspd_current);
      const gust = getVal(data.wspdhi_day_max);
      const rainRate = getVal(data.rainrate_current);
      const rainToday = getVal(data.rain_day_total);

      const lastUpdate = getTime(data.temp_current) || Date.now();

      const fmt = (val) =>
        val == null || isNaN(val) ? null : (val % 1 === 0 ? val.toFixed(1) : val);
      ...
      const obj = {
        ...
        LAT: st.lat,
        LON: st.lon,
        TS: new Date(lastUpdate).toISOString()
      };
    ...
    console.error("Errore Weathercloud:", err);
    res.status(500).json({ error: err.message });
  }
});
